...
export function CopyCodeButton({ code, brandName, slug }: Props) {
  ...
  const shareText = encodeURIComponent(`Use my referral code *${code}* to get signup benefits on *${brandName}*! Detailed guide here: https://referbenefits.co.in/${slug}`);
    ...
    <div className={styles.container}>
      ...
      <div className={styles.actions}>
        <button
          ...
          onClick={handleCopy}
          aria-label={copied ? `Referral code ${code} copied to clipboard` : "Copy referral code"}
        >
          {copied ? (
            <>
      ...
      {/* Permanent WCAG Status Region for Screen Readers */}
      <div className="sr-only" role="status" aria-live="polite">
        {copied ? `Referral code ${code} for ${brandName} copied to clipboard.` : ""}
      </div>
    </div>
  );
}
